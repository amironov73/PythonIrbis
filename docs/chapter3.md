...
Каждый экземпляр класса `Record` соответствует одной записи в базе данных ИРБИС. 

Запись характеризуется:

* **База данных** -- имя базы данных, из которой загружена данная запись. Для вновь созданных записей `None`. После отправки записи на сервер поле `database` выставляется автоматически.
...
* **Статус** -- состояние записи: удалена, отсутствует и т. д. Для вновь созданных записей 0. После отправки записи на сервер поле `status` выставляется автоматически. Обратите внимание, при реорганизации базе данных логически удалённые записи могут пропасть из неё.
...
* **Поля** -- запись содержит произвольное количество полей. Технически их может быть 0, но на практике это означает сбой системы.
...
Класс `Record` содержит следующие поля:
...
Поле записи характеризуется числовой меткой в диапазоне от 1 до 2147483647 (на практике встречаются коды от 1 до 9999) и содержит значение до первого разделителя (опционально) и произвольное количество подполей (см. `SubField`).
...
Хотя технически поле может содержать одновременно и значение до первого разделителя, и подполя, но на практике такая ситуация означает сбой. В стандарте MARC поле содержит либо значение либо подполя. Также в стандарте MARC поля с метками 1-9 являются служебными и имеют фиксированную структуру.
...
Класс `Field` содержит следующие поля:
